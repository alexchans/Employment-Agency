...
function StaffRER() {
    ...
    useEffect(() => {
        const fetchUsernames = async () => {
            const response = await axios.get(`http://localhost:8080/api/employerRequests/all`);
    ...
    const handleApprove = async (username) => {
        await axios.post(`http://localhost:8080/api/employer/create/${username}`);
        window.location.reload();
    };

    const handleDeny = async (username) => {
        await axios.put(`http://localhost:8080/api/employer/deleteRequest/${username}`);
        window.location.reload();
    };
